...
-- Initialize GTK. When LuaGObject is loaded as a plugin into a host
-- application that already called gtk_init() itself (e.g. Pluma, via
-- plugins/lualoader), calling disable_setlocale() this late is a no-op
-- that only produces a "must be called before gtk_init()" warning; skip
-- it if a default display already exists (a reliable sign gtk_init()
-- already ran). init_check() itself is safe to call again either way.
