...
peas_plugin_loader_lua_initialize (PeasPluginLoader *loader)
{
...
#ifdef PEAS_LUA_EXTRA_PATH
  /* Pluma bundles its own LuaGObject runtime (built against
   * girepository-2.0) instead of relying on a system-wide 'lgi'/
   * 'LuaGObject' install; point this Lua state at it before requiring
   * the module below. */
  {
    gchar *snippet = g_strdup_printf (
  ...
  if (!peas_lua_utils_load_resource (L, "strict.lua", 0, 0) ||
      !peas_lua_utils_require (L, "LuaGObject") ||
